...
export function TraderConfigViewModal({
  ...
  const signalSourceType = traderData.signal_source_type || 'ai'
  const isCopyMode = signalSourceType !== 'ai'
  const signalSourceLabel =
    signalSourceType === 'hyperliquid_wallet'
      ? 'Hyperliquid 钱包'
      : signalSourceType === 'okx_wallet'
        ? 'OKX 钱包'
        : 'AI 决策'
  const copyConfig = traderData.copy_trading_config
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-[#0B0E11] border border-[#2B3139] rounded-lg p-5">
            ...
            <div className="space-y-3">
              ...
              />
              <InfoRow label="信号源" value={signalSourceLabel} />
              {isCopyMode && (
                <InfoRow
                  label="复制钱包/ID"
                  value={traderData.signal_source_value || '-'}
                  copyable
                  fieldName="signal_source_value"
                />
              )}
            </div>
          </div>
          ...
          {isCopyMode && copyConfig && (
            <div className="bg-[#0B0E11] border border-[#2B3139] rounded-lg p-5">
              <h3 className="text-lg font-semibold text-[#EAECEF] mb-4 flex items-center gap-2">
                🧩 定比跟单配置
              </h3>
              <div className="space-y-3">
                <InfoRow label="跟随开仓" value={copyConfig.follow_open} />
                <InfoRow label="跟随加仓" value={copyConfig.follow_add} />
                <InfoRow label="跟随减仓" value={copyConfig.follow_reduce} />
                <InfoRow
                  label="跟单系数"
                ...
                <InfoRow
                  label="金额限制"
                  value={`最小 $${copyConfig.min_amount || 0} / 最大 $${copyConfig.max_amount || 0}`}
                />
                <InfoRow label="同步杠杆" value={copyConfig.sync_leverage} />
                <InfoRow
                  label="同步保证金模式"
                  value={copyConfig.sync_margin_mode}
                />
              </div>
            </div>
          )}
