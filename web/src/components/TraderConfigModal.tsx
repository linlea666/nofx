...
const defaultCopyTradingConfig: CopyTradingConfig = {
  follow_open: true,
  follow_add: true,
  follow_reduce: true,
  follow_ratio: 100,
...
export function TraderConfigModal({
  ...
  const [formData, setFormData] = useState<TraderConfigData>({
    ...
    signal_source_type: 'ai',
    signal_source_value: '',
    copy_trading_config: { ...defaultCopyTradingConfig },
  })
  ...
  useEffect(() => {
    if (traderData) {
      setFormData({
        ...traderData,
        signal_source_type: traderData.signal_source_type || 'ai',
        signal_source_value: traderData.signal_source_value || '',
        copy_trading_config:
          traderData.copy_trading_config || { ...defaultCopyTradingConfig },
      })
      // 设置已选择的币种
  ...
  const isCopyMode = formData.signal_source_type !== 'ai'
  const copyConfig = formData.copy_trading_config || {
  ...
  const handleCopyConfigChange = (
    ...
    setFormData((prev) => ({
      ...prev,
      copy_trading_config: {
        ...(prev.copy_trading_config || { ...defaultCopyTradingConfig }),
        [field]: value,
      },
    }))
  }
  ...
  const handleSave = async () => {
    ...
    try {
      const saveData: CreateTraderRequest = {
        ...
        signal_source_type: formData.signal_source_type,
        signal_source_value: formData.signal_source_value,
        copy_trading_config:
          formData.signal_source_type === 'ai'
            ? null
            : formData.copy_trading_config,
      }
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    value={formData.signal_source_type}
                    onChange={(e) =>
                      handleInputChange('signal_source_type', e.target.value)
                    }
                    className="w-full px-3 py-2 bg-[#0B0E11] border border-[#2B3139] rounded text-[#EAECEF] focus:border-[#F0B90B] focus:outline-none"
                  >
                    <option value="ai">AI 决策</option>
                    <option value="hyperliquid_wallet">
                      Hyperliquid 钱包
                    </option>
                ...
                {isCopyMode && (
                  <div>
                    ...
                    <input
                      type="text"
                      value={formData.signal_source_value}
                      onChange={(e) =>
                        handleInputChange('signal_source_value', e.target.value)
                      }
                      className="w-full px-3 py-2 bg-[#0B0E11] border border-[#2B3139] rounded text-[#EAECEF] focus:border-[#F0B90B] focus:outline-none"
                      placeholder="输入钱包地址或 OKX uniqueName"
                    />
                  </div>
                )}
              </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-3 gap-4">
                  <label className="flex items-center gap-2 text-sm text-[#EAECEF]">
                    <input
                      type="checkbox"
                      checked={copyConfig.follow_open}
                      onChange={(e) =>
                        handleCopyConfigChange('follow_open', e.target.checked)
                      }
                      className="w-4 h-4"
                    />
                    跟随开仓
                  ...
                  <label className="flex items-center gap-2 text-sm text-[#EAECEF]">
                    <input
                      type="checkbox"
                      checked={copyConfig.follow_add}
                      onChange={(e) =>
                        handleCopyConfigChange('follow_add', e.target.checked)
                      }
                      className="w-4 h-4"
                    />
                    跟随加仓
                  ...
                  <label className="flex items-center gap-2 text-sm text-[#EAECEF]">
                    <input
                      type="checkbox"
                      checked={copyConfig.follow_reduce}
                      onChange={(e) =>
                        handleCopyConfigChange('follow_reduce', e.target.checked)
                      }
                      className="w-4 h-4"
                    />
                    跟随减仓
                ...
                <div className="grid grid-cols-3 gap-4">
                  <label className="flex items-center gap-2 text-sm text-[#EAECEF]">
                    <input
                      type="checkbox"
                      checked={copyConfig.sync_leverage}
                      onChange={(e) =>
                  ...
                  <label className="flex items-center gap-2 text-sm text-[#EAECEF]">
                    <input
                      type="checkbox"
                      checked={copyConfig.sync_margin_mode}
                      onChange={(e) =>
                        handleCopyConfigChange(
                          'sync_margin_mode',
                          e.target.checked
                ...
                <p className="text-xs text-[#848E9C]">
                  系统仅做全量复制提示，不会强行阻断交易；请根据自身风险偏好调整参数。
                </p>
              </div>
            </div>
          )}
