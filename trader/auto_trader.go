...
type AutoTraderConfig struct {
	...
	// 信号源
	SignalSourceType  string            // ai / hyperliquid_wallet / okx_wallet
	SignalSourceValue string            // 钱包地址或 uniqueName
	CopyTradingConfig CopyTradingConfig // 跟单配置
...
func NewAutoTrader(config AutoTraderConfig, database interface{}, userID string) (*AutoTrader, error) {
	...
	if config.SignalSourceType == "" {
		config.SignalSourceType = "ai"
	} else {
		config.SignalSourceType = strings.ToLower(config.SignalSourceType)
	}
	config.CopyTradingConfig = normalizeCopyTradingConfig(config.CopyTradingConfig)
	...
	return &AutoTrader{
		...
		signalSourceType:      config.SignalSourceType,
		signalSourceValue:     config.SignalSourceValue,
		copyTradingConfig:     config.CopyTradingConfig,
		config:                config,
		trader:                trader,
		mcpClient:             mcpClient,
...
func (at *AutoTrader) Run() error {
	...
	if at.signalSourceType != "ai" {
		log.Printf("🛰 [%s] 进入复制交易模式，信号源: %s (%s)", at.name, at.signalSourceType, at.signalSourceValue)
		return at.runCopyTradingLoop()
	}
...
func (at *AutoTrader) GetStatus() map[string]interface{} {
	...
	return map[string]interface{}{
		...
		"signal_source_type": at.signalSourceType,
		"signal_source_value": at.signalSourceValue,
	}
}
