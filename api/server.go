...
type CreateTraderRequest struct {
	...
	SignalSourceType     string                   `json:"signal_source_type"`
	SignalSourceValue    string                   `json:"signal_source_value"`
	CopyTradingConfig    *CopyTradingConfigPayload `json:"copy_trading_config"`
}

func normalizeSignalSourceType(value string) string {
	switch strings.ToLower(value) {
	case "hyperliquid_wallet":
		return "hyperliquid_wallet"
	case "okx_wallet":
		return "okx_wallet"
	default:
		return "ai"
...
func marshalCopyTradingConfigPayload(payload *CopyTradingConfigPayload) string {
	cfg := &CopyTradingConfigPayload{
		FollowOpen:     true,
		FollowAdd:      true,
		FollowReduce:   true,
		FollowRatio:    100,
	...
	if payload != nil {
		if payload.FollowRatio > 0 {
			cfg.FollowRatio = payload.FollowRatio
		}
		if payload.MinAmount > 0 {
			cfg.MinAmount = payload.MinAmount
		}
		if payload.MaxAmount > 0 {
			cfg.MaxAmount = payload.MaxAmount
		}

		cfg.FollowOpen = payload.FollowOpen
		cfg.FollowAdd = payload.FollowAdd
		cfg.FollowReduce = payload.FollowReduce
		cfg.SyncLeverage = payload.SyncLeverage
		cfg.SyncMarginMode = payload.SyncMarginMode
	}
...
func parseCopyTradingConfig(raw string) map[string]interface{} {
...
func (s *Server) handleCreateTrader(c *gin.Context) {
	...
	signalSourceValue := ""
	copyTradingConfigJSON := ""
	if signalSourceType != "ai" {
		if strings.TrimSpace(req.SignalSourceValue) == "" {
			c.JSON(http.StatusBadRequest, gin.H{"error": "复制交易必须提供钱包/交易员标识"})
			return
		}
		signalSourceValue = strings.TrimSpace(req.SignalSourceValue)
		copyTradingConfigJSON = marshalCopyTradingConfigPayload(req.CopyTradingConfig)
	}
	...
	trader := &config.TraderRecord{
		...
		SignalSourceValue:    signalSourceValue,
		CopyTradingConfig:    copyTradingConfigJSON,
		IsRunning:            false,
	}
...
func (s *Server) handleUpdateTrader(c *gin.Context) {
	...
	if signalSourceType == "ai" {
		...
		value := strings.TrimSpace(req.SignalSourceValue)
		if value == "" {
			value = existingTrader.SignalSourceValue
		}
		if value == "" {
			c.JSON(http.StatusBadRequest, gin.H{"error": "复制交易必须提供钱包/交易员标识"})
			return
...
func (s *Server) handleTraderList(c *gin.Context) {
	...
	for _, trader := range traders {
		...
		result = append(result, map[string]interface{}{
			...
			"signal_source_type":  trader.SignalSourceType,
			"signal_source_value": trader.SignalSourceValue,
			"copy_trading_config": parseCopyTradingConfig(trader.CopyTradingConfig),
		})
	}
...
func (s *Server) handleGetTraderConfig(c *gin.Context) {
	...
	result := map[string]interface{}{
		...
		"is_running":            isRunning,
		"signal_source_type":    traderConfig.SignalSourceType,
		"signal_source_value":   traderConfig.SignalSourceValue,
		"copy_trading_config":   parseCopyTradingConfig(traderConfig.CopyTradingConfig),
	}
