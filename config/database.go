...
func (d *Database) createTables() error {
	queries := []string{
		...
		`CREATE TABLE IF NOT EXISTS traders (
			...
			signal_source_type TEXT DEFAULT 'ai',
			signal_source_value TEXT DEFAULT '',
			copy_trading_config TEXT DEFAULT '',
			created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
			updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
			FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
		)`,
	...
	alterQueries := []string{
		...
		`ALTER TABLE traders ADD COLUMN signal_source_type TEXT DEFAULT 'ai'`,
		`ALTER TABLE traders ADD COLUMN signal_source_value TEXT DEFAULT ''`,
		`ALTER TABLE traders ADD COLUMN copy_trading_config TEXT DEFAULT ''`,
...
type TraderRecord struct {
	...
	SignalSourceType     string    `json:"signal_source_type"`     // 信号源类型
	SignalSourceValue    string    `json:"signal_source_value"`    // 信号源标识（钱包地址等）
	CopyTradingConfig    string    `json:"copy_trading_config"`    // 跟单配置（JSON字符串）
...
func (d *Database) CreateTrader(trader *TraderRecord) error {
	_, err := d.db.Exec(`
		INSERT INTO traders (id, user_id, name, ai_model_id, exchange_id, initial_balance, scan_interval_minutes, is_running, btc_eth_leverage, altcoin_leverage, trading_symbols, use_coin_pool, use_oi_top, signal_source_type, signal_source_value, copy_trading_config, custom_prompt, override_base_prompt, system_prompt_template, is_cross_margin)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, trader.ID, trader.UserID, trader.Name, trader.AIModelID, trader.ExchangeID, trader.InitialBalance, trader.ScanIntervalMinutes, trader.IsRunning, trader.BTCETHLeverage, trader.AltcoinLeverage, trader.TradingSymbols, trader.UseCoinPool, trader.UseOITop, trader.SignalSourceType, trader.SignalSourceValue, trader.CopyTradingConfig, trader.CustomPrompt, trader.OverrideBasePrompt, trader.SystemPromptTemplate, trader.IsCrossMargin)
	return err
}
...
func (d *Database) GetTraders(userID string) ([]*TraderRecord, error) {
	rows, err := d.db.Query(`
		SELECT id, user_id, name, ai_model_id, exchange_id, initial_balance, scan_interval_minutes, is_running,
		       ...
		       COALESCE(signal_source_type, 'ai') as signal_source_type,
		       COALESCE(signal_source_value, '') as signal_source_value,
		       COALESCE(copy_trading_config, '') as copy_trading_config,
	...
	for rows.Next() {
		...
		err := rows.Scan(
			...
			&trader.SignalSourceType, &trader.SignalSourceValue, &trader.CopyTradingConfig,
...
func (d *Database) UpdateTrader(trader *TraderRecord) error {
	_, err := d.db.Exec(`
		UPDATE traders SET
			...
			signal_source_type = ?, signal_source_value = ?, copy_trading_config = ?,
...
func (d *Database) GetTraderConfig(userID, traderID string) (*TraderRecord, *AIModelConfig, *ExchangeConfig, error) {
	...
	err := d.db.QueryRow(`
		SELECT
			...
			COALESCE(t.signal_source_type, 'ai') as signal_source_type,
			COALESCE(t.signal_source_value, '') as signal_source_value,
			COALESCE(t.copy_trading_config, '') as copy_trading_config,
